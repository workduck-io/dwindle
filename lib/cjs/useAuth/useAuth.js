...
var useAuth = function () {
                ...
                if (uPool) {
                    ...
                    nuser_1.getSession(wrapErr(function (sess) {
                        if (sess) {
                            ...
                            reject("There's no User session");
                    }));
                }
                else
                    reject('Not in user pool');
            }
            else
        ...
        return new Promise(function (resolve, reject) {
            if (uPool) {
                ...
                var cognitoUser = new amazon_cognito_identity_js_1.CognitoUser({
                    Username: email,
                    Pool: new amazon_cognito_identity_js_1.CognitoUserPool(uPool),
                });
    ...
    var verifyForgotPassword = function (verificationCode, newPassword) {
        return new Promise(function (resolve, reject) {
            if (email) {
                if (uPool) {
                    console.log({ email: email, uPool: uPool });
                    console.log({ verificationCode: verificationCode, newPassword: newPassword });
                    var cognitoUser = new amazon_cognito_identity_js_1.CognitoUser({
                        Username: email,
                        Pool: new amazon_cognito_identity_js_1.CognitoUserPool(uPool),
                    });
