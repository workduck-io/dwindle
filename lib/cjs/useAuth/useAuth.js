        ...
        return new Promise(function (resolve, reject) {
            try {
                if (userCred) {
                    if (uPool) {
                        var userPool = new amazon_cognito_identity_js_1.CognitoUserPool(uPool);
                        var nuser_2 = new amazon_cognito_identity_js_1.CognitoUser({ Username: userCred.email, Pool: userPool });
        ...
        return new Promise(function (resolve, reject) {
            try {
                if (userCred) {
                    if (uPool) {
                        var userPool = new amazon_cognito_identity_js_1.CognitoUserPool(uPool);
                        var nuser_3 = new amazon_cognito_identity_js_1.CognitoUser({ Username: userCred.email, Pool: userPool });
                        nuser_3.getSession(
                        // @ts-ignore
