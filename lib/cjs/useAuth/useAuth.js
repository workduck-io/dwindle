...
var useAuth = function () {
    ...
    var getUserCred = (0, useAuthStore_1.default)(function (store) { return store.getUserCred; });
    ...
    var googleSignIn = function (code, clientId, redirectURI) {
        return new Promise(function (resolve, reject) {
            try {
                var dataPayload = qs_1.default.stringify({
                ...
                (0, axios_1.default)(config)
                    ...
                    var decodedIdToken = (0, jwt_decode_1.default)(tripletTokens === null || tripletTokens === void 0 ? void 0 : tripletTokens.id_token);
                    var nUCred = {
                    ...
                    if (uPool) {
                        ...
                        var nuser = new amazon_cognito_identity_js_1.CognitoUser({ Username: nUCred.username, Pool: userPool });
                        var uSession = new amazon_cognito_identity_js_1.CognitoUserSession({
                            AccessToken: new amazon_cognito_identity_js_1.CognitoAccessToken({ AccessToken: tripletTokens.access_token }),
                            IdToken: new amazon_cognito_identity_js_1.CognitoIdToken({ IdToken: tripletTokens.id_token }),
                            RefreshToken: new amazon_cognito_identity_js_1.CognitoRefreshToken({ RefreshToken: tripletTokens.refresh_token }),
                        });
    ...
    var refreshToken = function () {
        return new Promise(function (resolve, reject) {
            ...
            console.log('Bharosa to nahi hai mujhe');
            console.log("UserCred: ".concat(userCred, " | uPool: ").concat(uPool));
            if (uCred) {
                console.log('User pool nahi mila kya usko?');
                if (uPool) {
                    console.log('Idhar to aa hi gaya tha log ka load tha?');
                    var userPool = new amazon_cognito_identity_js_1.CognitoUserPool(uPool);
                    var nuser_1 = new amazon_cognito_identity_js_1.CognitoUser({ Username: uCred.username, Pool: userPool });
                    nuser_1.getSession(wrapErr(function (sess) {
                        console.log('Dekho idhar to aaya nahi');
                        if (sess) {
                            console.log('Agar idhar aa gaya to badi baat hogi kaafi');
                            var refreshToken_ = sess.getRefreshToken();
                            nuser_1.refreshSession(refreshToken_, function (err, session) {
                                console.log('Aur idhar to mashallah');
                                if (err) {
                                    console.log(err);
                                    reject(err);
                                }
